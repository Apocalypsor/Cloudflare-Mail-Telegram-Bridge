...
export async function verifyTelegramLogin(botToken: string, data: TelegramLoginData): Promise<boolean> {
	...
	if (isNaN(authDate) || Date.now() / 1000 - authDate > TG_AUTH_MAX_AGE) return false;
	...
	const tokenHash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(botToken));
...
export async function generateSessionCookie(secret: string, telegramId: string): Promise<{ value: string; maxAge: number }> {
...
export async function verifySessionCookie(secret: string, cookie: string): Promise<string | null> {
	const parts = cookie.split(':');
	if (parts.length !== 3) return null;
	const [telegramId, tsStr, hmac] = parts;
	const ts = parseInt(tsStr, 10);
	if (isNaN(ts) || Date.now() / 1000 - ts > SESSION_TTL) return null;
