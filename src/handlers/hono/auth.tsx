...
import { generateSessionCookie, verifyTelegramLogin, type TelegramLoginData } from '@utils/session';
...
auth.get(ROUTE_LOGIN, async (c) => {
	...
	return c.html(<LoginPage botUsername={botInfo.username} returnTo={returnTo} />);
});

auth.get(ROUTE_LOGIN_CALLBACK, async (c) => {
	const { id, first_name, last_name, username, photo_url, auth_date, hash } = c.req.query() as Record<string, string>;
	const returnTo = c.req.query('return_to') || '/';
	...
	await upsertUser(c.env.DB, id, first_name, last_name, username, photo_url, isAdmin ? 1 : undefined);
