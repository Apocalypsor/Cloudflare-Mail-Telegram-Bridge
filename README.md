...
5. 每条消息附带 ⭐ **星标**、🚫 **垃圾**、📥 **归档**、🔄 **刷新**按钮（inline keyboard）。星标同时自动标记已读；标记垃圾会将邮件移到垃圾邮件文件夹并删除 Telegram 消息；归档将邮件移出收件箱（Outlook 进 Archive、IMAP 进自定义文件夹、Gmail 需先在账号详情里指定一个标签）；刷新会重新拉取邮件并执行 LLM 分析。
